...
f = open('Transformer.txt','a+',encoding='utf-8')
e = open('Translation.txt','w+',encoding='utf-8')
g=open('R.txt','w+',encoding='utf-8')
gg=open('reference.txt','w+',encoding='utf-8')
# 初始化参数设置
UNK = 0  # 未登录词的标识符对应的词典id
PAD = 1  # padding占位符对应的词典id
BATCH_SIZE =16   # 每批次训练数据数量
EPOCHS = 100  # 训练轮数
LAYERS = 6  # transformer中堆叠的encoder和decoder block层数
H_NUM = 8  # multihead attention hidden个数
D_MODEL = 128  # embedding维数
...
train_code_file = "fold_1/train/train.token.code"
train_comment_file = "fold_1/train/train.token.code"
test_code_file = "fold_1/test/test.token.code"
test_comment_file = "fold_1/test/test.token.code"
eval_code_file = "fold_1/eval/dev.token.code"                    # #加入验证集
eval_comment_file = "fold_1/eval/dev.token.code"                 # #加入验证集
SAVE_FILE = 'save/model1.pt'  # 模型保存路径(注意如当前目录无save文件夹需要自己创建)
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print('cuda is:%s' % torch.cuda.is_available())
...
class PrepareData:
    ...
    def __init__(self, train_code_file, train_comment_file , test_code_file, test_comment_file, eval_code_file, eval_comment_file):  # #加入验证集

        # 读取数据 并分词
        # self.train_en, self.train_cn = self.load_data(train_file)
        # self.dev_en, self.dev_cn = self.load_data(dev_file)
        self.train_en, self.train_cn = self.load_data(train_code_file, train_comment_file)   # #en是数据，cn是标签，分词后的
        self.dev_en, self.dev_cn = self.load_data(test_code_file, test_comment_file)  # #对测试集分词，加上停止词de序列
        self.eval_en, self.eval_cn = self.load_data(eval_code_file, eval_comment_file)          # #加入验证集
        ...
        self.eval_en, self.eval_cn = self.wordToID(self.eval_en, self.eval_cn, self.en_word_dict, self.cn_word_dict)   # #加入验证集
        ...
        self.eval_data = self.splitBatch(self.eval_en, self.eval_cn, BATCH_SIZE)   # #加入验证集

    def load_data(self,code_file, comment_file):
        comment = []
        code = []
        # num_examples = 0
        with open(code_file, 'r', encoding='utf-8') as f:
            for code_line in f:
                code.append(["BOS"] + word_tokenize(code_line.lower()) + ["EOS"])
        with open(comment_file, "r", encoding='utf-8') as f:
            for comment_line in f:
                comment.append(["BOS"] +word_tokenize(comment_line.lower()) + ["EOS"])
...
def run_epoch(data, model, loss_compute, epoch):
    ...
    for i, batch in enumerate(data):
        ...
        if i % 50 == 1:
            ...
            epoch, i - 1, loss / batch.ntokens, (tokens.float() / elapsed / 1000.)),file=f)
            print("Epoch %d Batch: %d Loss: %f Tokens per Sec: %fs" % (
...
def train(data, model, criterion, optimizer):
    ...
    for epoch in range(EPOCHS):
        ...
        print('>>>>> Evaluate')
        dev_loss = run_epoch(data.eval_data, model, SimpleLossCompute(model.generator, criterion, None), epoch)
        print('<<<<< Evaluate loss: %f' % dev_loss,file=f)
...
data = PrepareData(train_code_file, train_comment_file, test_code_file, test_comment_file, eval_code_file, eval_comment_file)  # #加入验证集
src_vocab = len(data.en_word_dict)
tgt_vocab = len(data.cn_word_dict)
print("src_vocab %d" % src_vocab,file=f)
print("src_vocab %d" % src_vocab)
print("tgt_vocab %d" % tgt_vocab,file=f)
...
# 训练
print(">>>>>>> start train",file=f)
print(">>>>>>> start train")
train_start = time.time()
criterion = LabelSmoothing(tgt_vocab, padding_idx=0, smoothing=0.0)

optimizer = NoamOpt(D_MODEL, 1, 2000, torch.optim.Adam(model.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
print("model is:",os.path.exists(SAVE_FILE))
if os.path.exists(SAVE_FILE):
    model.load_state_dict(torch.load(SAVE_FILE))
    aaa= run_epoch(data.dev_data, model, SimpleLossCompute(model.generator, criterion, None), 999)
train(data, model, criterion, optimizer)
print("<<<<<<< finished train, cost {time.time()-train_start:.4f} seconds",file=f)
print("<<<<<<< finished train, cost {time.time()-train_start:.4f} seconds")
...
def evaluate(data, model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data.dev_en)):
            ...
            print(cn_sent)
            print("".join(cn_sent),file=e)

            print(cn_sent[4:-4])
            print(cn_sent[4:-4],file=gg)
            # sn_sent = " ".join([data.cn_index_dict[w+1] for w in data.dev_cn[i-1]])
            # print("".join(sn_sent), file=gg)
            ...
            for j in range(1, out.size(1)):
                ...
                # 否则终止遍历
                    ...
                    break
            # 打印模型翻译输出的中文句子结果
            print("translation: %s" % " ".join(translation),file=e)
            print("%s"%' '.join(translation),file=g)

# 预测
# 加载模型
model.load_state_dict(torch.load(SAVE_FILE))
# 开始预测
print(">>>>>>> start evaluate",file=e)
evaluate_start = time.time()
evaluate(data, model)
print(f"<<<<<<< finished evaluate, cost {time.time()-evaluate_start:.4f} seconds",file=e)
